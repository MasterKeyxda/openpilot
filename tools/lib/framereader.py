...
def read_fcamera_stream(frames,
                        gpu=-1, pix_fmt="yuv420p",
                        crop_to=None, resize_to=None, read_func=None, flip=False):
  """sample frames from lots of fcamera files fast
     Returns a generator which yields uncompressed yuv420p frames.

     Inputs:
      frames: sequence of pairs (cookie, frame_info)...
              cookie is the caller's result identifier, frame_info is a tuple of either
              (camera path, frame index) or (camera path, frame index, number of frames).
      gpu: The device number of the GPU to use for cuvid decoding.
           Use -1 to find any useable GPU, or None for CPU.
      pix_fmt: The FFmpeg pix_fmt that the output bytes will have.
      crop_to: A tuple (top_left_x, top_left_y, width, height) specifying the coordinates
               of frames to return. None specifies no crop. crop_to cannot be used at the
               same time as resize_to.
      resize_to: A tuple (width, height) specifying dimensions to which the image should be
                 resized before returning. resize_to cannot be used at the same time as crop_to.
      read_func: A function f(file, size, cookie) -> buff that reads size bytes from a file and
                 produces a buffer

     Returns:
      A sequence of ( (cookie A, frame A), (cookie B, frame B) ...).
      The returned cookies can be used to associate input frame_infos with output frames.

      frame_infos corresponding to multiple frames are flattened in the output.
      Their frames are always consecutive and ordered sequentially.
        (A, (_, _, 3)), (B, (_, _, 2))... -> [A, _], [A, _], [A, _], [B, _], [B, _]...
  """

  # This will be slow without the index cache.
  assert os.path.exists("/cache")

  raw_out_dim = 874, 1164

  if crop_to is None:
    crop_args = []
    out_dim = raw_out_dim
  else:
    assert len(crop_to) == 4, len(crop_to)
    crop_spec = (crop_to[1], raw_out_dim[0] - (crop_to[1] + crop_to[3]), crop_to[0],
                 raw_out_dim[1] - (crop_to[0] + crop_to[2]))
    crop_args = ["-crop", "{}x{}x{}x{}".format(*crop_spec)]
    out_dim = (crop_to[3], crop_to[2])

  if resize_to is None:
    # out_dim = raw_out_dim
    out_dim_str = "ih", "iw"
  else:
    out_dim = (resize_to[1], resize_to[0])
    out_dim_str = out_dim


  if pix_fmt == "yuv420p":
    out_size = out_dim[0] * out_dim[1] * 3 // 2
  elif pix_fmt == "rgb24":
    out_size = out_dim[0] * out_dim[1] * 3
  else:
    raise NotImplementedError(pix_fmt)

  start_args = [
     os.path.expanduser("~/one/external/ffmpeg/bin/ffmpeg_cuda"),
     #"-v", "fatal",
     "-vsync", "0",
     "-threads", "2",
     "-f", "hevc"]

  select_pipe_path = os.path.join(tempfile.mkdtemp(), "f")
  os.mkfifo(select_pipe_path)
  select_pipe_fd = os.open(select_pipe_path, os.O_RDWR)
  select_arg = "select=f={}".format(select_pipe_path)
  ...
  if gpu is not None and int(gpu) >= 0:
    # We set CUDA_VISIBLE_DEVICES here because it seems to reduce the
    # frequency of "freezes" where one instance of ffmpeg makes no progress
    # until all others have stopped.
    ...
    cmd = (start_args + [
     ...
     crop_args + [
     "-i", "pipe:0",
     "-vf", "{},scale_npp=w={}:h={}:format=yuv420p,hwdownload,format=yuv420p".format(
        select_arg, out_dim_str[1], out_dim_str[0]),
     "-f", "rawvideo"] +
     ([] if pix_fmt == "yuv420p" else ["-pix_fmt", pix_fmt])
     + ["pipe:1"])
  else:
    if resize_to is None:
      scale_arg = ""
    else:
      scale_arg = ",scale=w={}:h={}".format(out_dim_str[1], out_dim_str[0])

    cmd = (start_args +
     crop_args + [
     "-i", "pipe:0",
     "-vf", "{}{}".format(select_arg, scale_arg),
     "-f", "rawvideo",
     "-pix_fmt", pix_fmt,
     "pipe:1"])

  # Contains tuples (cookie, count)
  cookie_queue = queue.Queue()
  cookie = None

  # We get errors about non-monotonically increasing dts that may be unavoidable.
  # https://ffmpeg.org/pipermail/ffmpeg-devel/2010-September/102086.html
  proc = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                          env=env, close_fds=True, preexec_fn=_set_pdeathsig())

  t = threading.Thread(
    target=feed_ffmpeg_fcamera_input,
    args=(frames, proc.stdin, select_pipe_fd, cookie_queue))
  t.daemon = True

  # Use default read function if the caller did not provide one.
  if read_func is None:
    read_func = read_file_check_size
  ...
  try:
    ...
    total_frames_out = 0
    while True:
      cookie, count = cookie_queue.get()
      if cookie is _FCAMERA_FEED_SUCCESS:
        break
    ...
    os.close(select_pipe_fd)
    os.remove(select_pipe_path)
    os.rmdir(os.path.dirname(select_pipe_path))

    # FFmpeg will exit with error on empty input streams.
    draining = [True]
    def drain(p):
      while draining[0]:
        p.stdout.read(512)
    d = threading.Thread(target=drain, args=(proc,))
    d.daemon = True
    d.start()
    try:
      ffmpeg_exit_code = proc.wait(5.)
  ...
  try:
    # Count the rest of the inputs in case ffmpeg crashed.
    while True:
      cookie_queue.get_nowait()
      total_frames_in += 1
  except queue.Empty:
    pass


  if cookie is not _FCAMERA_FEED_SUCCESS:
    raise Exception("Feed thread did not complete successfully")

  if total_frames_in != total_frames_out:
    raise Exception(
      "frames in != frames_out: {} != {}".format(total_frames_in, total_frames_out))

  if total_frames_in > 0:
    if ffmpeg_exit_code != 0:
      raise Exception("ffmpeg exited with {}".format(ffmpeg_exit_code))


def read_fcamera_batch(frame_infos, *args, **kwargs):
  """sample frames from lots of fcamera files fast
     See read_fcamera_stream for argument details.
  """
  # Return results in the order they were passed in.
  frame_infos = list(frame_infos)
  results = list(read_fcamera_stream(zip(range(len(frame_infos)), frame_infos), *args, **kwargs))
